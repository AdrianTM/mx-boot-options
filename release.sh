...
print_header() {
    echo -e "${BLUE}========================================${NC}"
    echo -e "${BLUE}  MX Boot Options Arch Release Script${NC}"
    echo -e "${BLUE}========================================${NC}"
    echo
}
...
get_latest_tag() {
    git tag -l | while read -r tag; do
        ...
        if [[ $clean_tag =~ ^[0-9]+\.[0-9]+(\.[0-9]+)?[a-z]*$ ]]; then
            printf '%s\n' "$tag"
        fi
    done | sort -V | tail -n1
}
...
update_aur_package() {
    ...
    local tarball_url="https://github.com/MX-Linux/mx-boot-options/archive/refs/tags/${version}.tar.gz"
